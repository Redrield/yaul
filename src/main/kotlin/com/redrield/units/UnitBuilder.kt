...
inline class UnitBuilder internal constructor(val value: Double) {
    val gram get() = SIUnit<Kilogram>(value / 1000.0)
    val second get() = SIUnit<Second>(value)
    val amp get() = SIUnit<Ampere>(value)
    val coulomb get() = SIUnit<Coulomb>(value)
    val volt get() = SIUnit<Volt>(value)
    val ohm get() = SIUnit<Ohm>(value)
    val newton get() = SIUnit<Newton>(value)
    val joule get() = SIUnit<Joule>(value)
    val hertz get() = SIUnit<Hertz>(value)
    val curvature get() = SIUnit<Curvature>(value)
    val meter get() = SIUnit<Meter>(value)

    val radian get() = SIUnit<Radian>(value)
    val degree get() = SIUnit<Radian>(Math.toRadians(value))
}
...
val Number.curvature get() = SIUnit<Curvature>(toDouble())
